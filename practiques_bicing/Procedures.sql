...
BEGIN
    ...
    WHILE i < num DO
        ...
        SELECT
            DATE_SUB(NOW(), INTERVAL FLOOR(RAND() * 720) MINUTE),  -- hora inici aleatòria
            NULL,                                                  -- trajecte encara no acabat
            1,
            FLOOR(1 + (RAND() * 50)),                              -- estació origen
            NULL,                                                  -- estació final desconeguda
            b.idBICICLETA,                                         -- bici aleatòria
            FLOOR(1 + (RAND() * 15))                               -- usuari aleatori
        FROM bicicleta b
        ORDER BY RAND()
        LIMIT 1;
